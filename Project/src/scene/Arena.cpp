...
static std::wstring widenPath(const char* path) {
    ...
    MultiByteToWideChar(CP_UTF8, 0, path, -1, buf.data(), len);
    return std::wstring(buf.data());
}

static bool ensureArenaGdiPlus() {
    if (!gArenaGdiInitialized) {
        Gdiplus::GdiplusStartupInput input;
        if (Gdiplus::GdiplusStartup(&gArenaGdiToken, &input, NULL) != Gdiplus::Ok) {
            return false;
        }
        _fpreset();
        gArenaGdiInitialized = true;
    }
    return true;
}

static GLuint loadTextureWithGdiPlus(const char* filename) {
    std::cout << "[Arena][GDI+] Begin load: " << (filename ? filename : "(null)") << std::endl;
    if (!filename || !ensureArenaGdiPlus()) {
        std::cout << "[Arena][GDI+] Startup/path precheck failed." << std::endl;
        return 0;
    }

    const std::wstring widePath = widenPath(filename);
    if (widePath.empty()) {
        std::cout << "[Arena][GDI+] widenPath failed." << std::endl;
        return 0;
    }
    std::cout << "[Arena][GDI+] Path converted." << std::endl;
...

    {
        ...
        if (bitmap.GetLastStatus() != Gdiplus::Ok) {
            std::cout << "[Arena][GDI+] Bitmap open failed: status=" << (int)bitmap.GetLastStatus() << std::endl;
            return 0;
        }
        std::cout << "[Arena][GDI+] Bitmap opened." << std::endl;

        width = (int)bitmap.GetWidth();
        height = (int)bitmap.GetHeight();
        if (width <= 0 || height <= 0) {
            std::cout << "[Arena][GDI+] Invalid dimensions: " << width << "x" << height << std::endl;
            return 0;
        }
        std::cout << "[Arena][GDI+] Dimensions: " << width << "x" << height << std::endl;

        Gdiplus::Rect rect(0, 0, width, height);
        Gdiplus::BitmapData locked = {};
        if (bitmap.LockBits(&rect, Gdiplus::ImageLockModeRead, PixelFormat32bppARGB, &locked) != Gdiplus::Ok) {
            std::cout << "[Arena][GDI+] LockBits failed." << std::endl;
            return 0;
        }
        std::cout << "[Arena][GDI+] LockBits ok. Stride=" << locked.Stride << std::endl;
        ...
        std::cout << "[Arena][GDI+] Pixel conversion done." << std::endl;
        bitmap.UnlockBits(&locked);
        std::cout << "[Arena][GDI+] UnlockBits ok." << std::endl;
    }
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, width, height, GL_RGB, GL_UNSIGNED_BYTE, &rgb[0]);
    std::cout << "[Arena][GDI+] Texture upload done." << std::endl;

    std::cout << "[Arena] Successfully loaded texture via GDI+: " << filename
              << " (" << width << "x" << height << ") | Anisotropy: " << maxAnisotropy << std::endl;
    return tex;
}
#endif
...
static GLuint loadTextureHelper(const char* filename) {
    ...
    if (!img.buffer) {
#ifdef _WIN32
        std::cout << "[Arena] Standard loader failed for " << filename << ". Trying GDI+ fallback..." << std::endl;
        GLuint tex = loadTextureWithGdiPlus(filename);
        if (tex != 0) {
            return tex;
        }
#endif
...
void Arena::drawTransparent() {
    ...
    // Faint seam guidance and local plastic wrinkles.
    glLineWidth(1.0f);
    glBegin(GL_LINES);
    for (int c = 1; c < 25; ++c) {
        float x = -8.10f + c * (16.20f / 24.0f);
        float phase = x * 0.41f;
        float z1 = wrapMinZ + 0.38f + 0.08f * sin(phase);
        float z2 = wrapMinZ + 1.28f + 0.06f * cos(phase * 1.7f);
        float z3 = wrapMaxZ - 1.30f + 0.05f * sin(phase * 1.2f);
        float z4 = wrapMaxZ - 0.44f + 0.08f * cos(phase * 0.9f);
        float xJitter = 0.03f * sin(phase * 2.3f);
        glColor4f(0.96f, 0.99f, 1.0f, 0.040f + 0.012f * sin(phase * 1.1f));
        glVertex3f(x + xJitter, wrapY + 0.015f + 0.002f * sin(phase), z1);
        glVertex3f(x - xJitter, wrapY + 0.017f + 0.002f * cos(phase), z2);
        glColor4f(0.92f, 0.97f, 1.0f, 0.030f + 0.010f * cos(phase * 1.4f));
        glVertex3f(x - xJitter * 0.7f, wrapY + 0.015f + 0.001f * cos(phase), z3);
        glVertex3f(x + xJitter * 0.6f, wrapY + 0.016f + 0.002f * sin(phase * 0.8f), z4);
    }
    for (int r = 1; r < 9; ++r) {
        float z = -2.40f + r * (4.80f / 8.0f);
        float phase = z * 1.27f;
        float x1 = wrapMinX + 0.72f + 0.08f * cos(phase);
        float x2 = wrapMinX + 3.00f + 0.12f * sin(phase * 1.5f);
        float x3 = wrapMaxX - 3.10f + 0.10f * cos(phase * 1.9f);
        float x4 = wrapMaxX - 0.86f + 0.08f * sin(phase * 0.8f);
        float zJitter = 0.024f * sin(phase * 1.7f);
        glColor4f(0.92f, 0.98f, 1.0f, 0.026f + 0.010f * cos(phase));
        glVertex3f(x1, wrapY + 0.015f + 0.001f * cos(phase), z + zJitter);
        glVertex3f(x2, wrapY + 0.016f + 0.001f * sin(phase), z - zJitter * 0.4f);
        glColor4f(0.88f, 0.95f, 1.0f, 0.020f + 0.008f * sin(phase * 1.2f));
        glVertex3f(x3, wrapY + 0.015f + 0.001f * sin(phase * 0.7f), z - zJitter * 0.5f);
        glVertex3f(x4, wrapY + 0.016f + 0.001f * cos(phase * 0.9f), z + zJitter);
    }
    glEnd();

    // Localized wrinkle clusters near lips and selected membrane intersections.
    glLineWidth(1.0f);
    for (int cluster = 0; cluster < 14; ++cluster) {
        float t = (float)cluster / 13.0f;
        float anchorX = wrapMinX + 1.0f + t * (wrapMaxX - wrapMinX - 2.0f);
        float anchorZ;
        if (cluster < 4) {
            anchorZ = wrapMaxZ - 0.34f - 0.16f * sin(1.9f * cluster);
        } else if (cluster < 8) {
            anchorZ = wrapMinZ + 0.36f + 0.14f * cos(1.4f * cluster);
        } else {
            anchorZ = wrapMinZ + 0.85f + (cluster - 8) * 0.33f + 0.08f * sin(cluster * 1.3f);
        }

        float basePhase = anchorX * 0.42f + anchorZ * 1.67f;
        for (int stroke = 0; stroke < 3; ++stroke) {
            float localPhase = basePhase + stroke * 1.37f;
            float length = 0.22f + 0.08f * (0.5f + 0.5f * sin(localPhase));
            float angle = 0.5f * sin(localPhase * 1.1f) + stroke * 0.55f;
            float dx = cos(angle) * length;
            float dz = sin(angle) * length;
            float bendX = 0.05f * sin(localPhase * 1.8f);
            float bendZ = 0.04f * cos(localPhase * 1.5f);
            float alpha = 0.035f + 0.016f * (0.5f + 0.5f * cos(localPhase * 0.9f));

            glColor4f(0.96f, 0.99f, 1.0f, alpha);
            glBegin(GL_LINE_STRIP);
            glVertex3f(anchorX - dx * 0.55f, wrapY + 0.020f + 0.002f * sin(localPhase), anchorZ - dz * 0.55f);
            glVertex3f(anchorX + bendX * 0.6f, wrapY + 0.024f + 0.002f * cos(localPhase * 0.8f), anchorZ + bendZ * 0.6f);
            glVertex3f(anchorX + dx * 0.52f + bendX, wrapY + 0.021f + 0.002f * sin(localPhase * 1.2f), anchorZ + dz * 0.52f + bendZ);
            glEnd();
        }
    }
    ...
    for (size_t i = 0; i < bubbles.size(); ++i) {
        if (bubbles[i].popped) continue;
        const float phaseA = bubbles[i].x * 0.61f + bubbles[i].z * 1.37f;
        const float phaseB = bubbles[i].x * 1.18f - bubbles[i].z * 0.73f;
        const float sizeJitter = 0.92f + 0.11f * sin(phaseA) + 0.04f * cos(phaseB * 1.7f);
        const float stretchX = 0.96f + 0.14f * sin(phaseB);
        const float stretchZ = 0.94f + 0.16f * cos(phaseA * 0.8f);
        const float skewX = 0.026f * sin(phaseA * 1.9f);
        const float skewZ = 0.022f * cos(phaseB * 1.6f);
        const float bubbleRadius = 0.158f * sizeJitter;
        float pressScale = 1.0f - bubbles[i].pressDepth * 0.76f;
        float bubbleY = wrapY + (0.100f + 0.010f * sin(phaseA * 0.7f)) * pressScale;
        float capHeight = 0.50f * pressScale * (0.90f + 0.08f * cos(phaseB));
        float rimY = wrapY + 0.030f + 0.006f * sin(phaseB * 0.5f);

        glColor4f(0.72f, 0.84f, 0.91f, 0.24f + 0.05f * cos(phaseA));
        glPushMatrix();
        glTranslatef(bubbles[i].x + skewX, bubbleY, bubbles[i].z + skewZ);
        glRotatef(8.0f * sin(phaseB), 0.0f, 1.0f, 0.0f);
        glScalef(stretchX, capHeight, stretchZ);
        glutSolidSphere(bubbleRadius, 18, 10);
        glPopMatrix();

        glColor4f(0.90f, 0.97f, 1.0f, 0.10f + 0.05f * sin(phaseA * 0.8f));
        glBegin(GL_LINE_STRIP);
        for (int s = 0; s <= 8; ++s) {
            float a = -0.25f + 1.20f * (float)s / 8.0f;
            float rimNoise = 1.0f + 0.08f * sin(a * 3.0f + phaseA) + 0.05f * cos(a * 5.0f - phaseB);
            glVertex3f(bubbles[i].x + skewX + cos(a) * bubbleRadius * 1.05f * stretchX * rimNoise,
                       rimY + 0.003f,
                       bubbles[i].z + skewZ + sin(a) * bubbleRadius * 1.05f * stretchZ * rimNoise);
        }
        glEnd();

        glColor4f(0.78f, 0.90f, 0.98f, 0.08f + 0.04f * cos(phaseB));
        glBegin(GL_LINE_STRIP);
        for (int s = 0; s <= 6; ++s) {
            float a = 3.35f + 0.95f * (float)s / 6.0f;
            float rimNoise = 1.0f + 0.05f * sin(a * 4.0f + phaseB);
            glVertex3f(bubbles[i].x + skewX + cos(a) * bubbleRadius * 0.98f * stretchX * rimNoise,
                       rimY - 0.001f,
                       bubbles[i].z + skewZ + sin(a) * bubbleRadius * 0.98f * stretchZ * rimNoise);
        }
        glEnd();

        glColor4f(1.0f, 1.0f, 1.0f, 0.16f + 0.08f * cos(phaseA * 1.3f));
        glBegin(GL_LINES);
        glVertex3f(bubbles[i].x - bubbleRadius * 0.45f, bubbleY + bubbleRadius * 0.30f, bubbles[i].z - bubbleRadius * 0.36f);
        glVertex3f(bubbles[i].x + bubbleRadius * 0.10f, bubbleY + bubbleRadius * 0.42f, bubbles[i].z - bubbleRadius * 0.56f);
        glVertex3f(bubbles[i].x - bubbleRadius * 0.12f, bubbleY + bubbleRadius * 0.18f, bubbles[i].z + bubbleRadius * 0.10f);
        glVertex3f(bubbles[i].x + bubbleRadius * 0.42f, bubbleY + bubbleRadius * 0.24f, bubbles[i].z + bubbleRadius * 0.32f);

        glColor4f(0.90f, 0.96f, 1.0f, 0.10f + 0.05f * sin(phaseB * 1.4f));
        glVertex3f(bubbles[i].x - bubbleRadius * 0.72f, bubbleY + bubbleRadius * 0.06f, bubbles[i].z - bubbleRadius * 0.05f);
        glVertex3f(bubbles[i].x - bubbleRadius * 0.22f, bubbleY + bubbleRadius * 0.11f, bubbles[i].z + bubbleRadius * 0.05f);
        glVertex3f(bubbles[i].x + bubbleRadius * 0.10f, bubbleY + bubbleRadius * 0.34f, bubbles[i].z - bubbleRadius * 0.22f);
        glVertex3f(bubbles[i].x + bubbleRadius * 0.38f, bubbleY + bubbleRadius * 0.40f, bubbles[i].z - bubbleRadius * 0.08f);
        glVertex3f(bubbles[i].x - bubbleRadius * 0.30f, bubbleY + bubbleRadius * 0.44f, bubbles[i].z + bubbleRadius * 0.18f);
        glVertex3f(bubbles[i].x - bubbleRadius * 0.06f, bubbleY + bubbleRadius * 0.48f, bubbles[i].z + bubbleRadius * 0.32f);
        glEnd();
    }
