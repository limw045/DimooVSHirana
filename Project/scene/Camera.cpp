...
void Camera::update(float dt, float char1X, float char1Z, float char2X, float char2Z, bool isUltActive) {
    ...
    if (isUltActive) {
        ...
        targetX = midX; 
        targetY = 1.1f + dist * 0.08f;  // 低机位（约角色胸部/眼睛高度），随拉远稍微抬高
        targetZ = 3.0f + dist * 0.45f;  // 动态焦距（保证两个角色始终同屏且清晰可见）
        
        targetLookAtX = midX;
        targetLookAtY = 0.85f;          // 锁定视线在角色胸部高度
        targetLookAtZ = midZ * 0.2f;    // 微调看点深度
    }

    // 限制相机边界，防止飞出纸盒外面
    targetX = clamp(targetX, -BOX_WIDTH / 2.0f + 1.2f, BOX_WIDTH / 2.0f - 1.2f);
    targetZ = clamp(targetZ, 1.8f, 7.2f);
